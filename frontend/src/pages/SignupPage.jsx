...
export default function SignupPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || t('signup.creating'));
  ...
  return (
    <div style={{ minHeight:'100vh', background:`radial-gradient(ellipse 80% 50% at 50% -10%,${C.p}10,transparent),#fafbfc`, fontFamily:"'Outfit','DM Sans',sans-serif", display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'40px 20px', position:'relative' }}>
      ...
      <div style={{ width:'100%', maxWidth:460, background:'#fff', borderRadius:24, padding:40, boxShadow:'0 20px 60px rgba(0,0,0,.08)', border:'1px solid #f1f5f9' }}>
        ...
        {step === 1 && (
          <>
            ...
            <p style={{ color:C.m, fontSize:14, margin:'0 0 28px' }}>{t('signup.subtitle')}</p>
            <div style={{ marginBottom:20 }}>
              <label style={{ fontSize:13, fontWeight:600, color:C.s, display:'block', marginBottom:6 }}>{t('signup.company')} *</label>
              <input value={form.company} onChange={e=>set('company',e.target.value)} placeholder={t('signup.company_ph')} style={inputStyle}
                onFocus={e=>e.target.style.borderColor=C.p} onBlur={e=>e.target.style.borderColor='#e2e8f0'}/>
            </div>
            <div style={{ marginBottom:20 }}>
              <label style={{ fontSize:13, fontWeight:600, color:C.s, display:'block', marginBottom:6 }}>{t('signup.fullname')} *</label>
              <input value={form.fullName} onChange={e=>set('fullName',e.target.value)} placeholder={t('signup.fullname_ph')} style={inputStyle}
                onFocus={e=>e.target.style.borderColor=C.p} onBlur={e=>e.target.style.borderColor='#e2e8f0'}/>
            </div>
            <div style={{ marginBottom:28 }}>
              <label style={{ fontSize:13, fontWeight:600, color:C.s, display:'block', marginBottom:6 }}>{t('signup.phone')}</label>
              <input value={form.phone} onChange={e=>set('phone',e.target.value)} placeholder={t('signup.phone_ph')} style={inputStyle}
                onFocus={e=>e.target.style.borderColor=C.p} onBlur={e=>e.target.style.borderColor='#e2e8f0'}/>
            </div>
            ...
            <button onClick={() => { if (form.company && form.fullName) { setError(''); setStep(2); } else setError(t('signup.error_fields')); }}
        ...
        {step === 2 && (
          <>
            ...
            <p style={{ color:C.m, fontSize:14, margin:'0 0 28px' }}>
              {t('signup.credentials_sub', { company: form.company })}
            </p>
            <div style={{ marginBottom:20 }}>
              <label style={{ fontSize:13, fontWeight:600, color:C.s, display:'block', marginBottom:6 }}>{t('signup.email')} *</label>
              <input type="email" value={form.email} onChange={e=>set('email',e.target.value)} placeholder={t('signup.email_ph')} style={inputStyle}
                onFocus={e=>e.target.style.borderColor=C.p} onBlur={e=>e.target.style.borderColor='#e2e8f0'}/>
            </div>
            <div style={{ marginBottom:8 }}>
              <label style={{ fontSize:13, fontWeight:600, color:C.s, display:'block', marginBottom:6 }}>{t('signup.password')} *</label>
              <div style={{ position:'relative' }}>
                <input type={showPwd ? "text" : "password"} value={form.password} onChange={e=>set('password',e.target.value)} placeholder={t('signup.password_ph')} style={inputStyle}
                  onFocus={e=>e.target.style.borderColor=C.p} onBlur={e=>e.target.style.borderColor='#e2e8f0'}/>
            ...
            <div style={{ marginBottom:28, display:'flex', flexWrap:'wrap', gap:6 }}>
              {pwdChecks.map(([ok,label],i) => (
                <span key={i} style={{ fontSize:11, padding:'3px 8px', borderRadius:6, background:ok?`${C.p}15`:'#f1f5f9', color:ok?C.p:'#94a3b8', fontWeight:600 }}>
                  {ok?'✓':'○'} {label}
                </span>
              ))}
            </div>
            {error && <div style={{ padding:'12px 16px', borderRadius:10, background:'#fef2f2', color:'#dc2626', fontSize:13, marginBottom:16, fontWeight:500 }}>{error}</div>}
            <button
              onClick={() => { if (!form.email || !passwordOk) setError(t('signup.error_creds')); else handleSubmit(); }}
              disabled={loading}
            ...
            <p style={{ color:'#94a3b8', fontSize:11, textAlign:'center', marginTop:16 }}>
              {t('signup.terms')} <a href="#" style={{ color:C.p }}>{t('signup.terms_cgv')}</a>{' '}
              {t('signup.terms_and')}{' '}<a href="#" style={{ color:C.p }}>{t('signup.terms_privacy')}</a>.
            </p>
          </>
        )}
        ...
        {step === 3 && (
          <div style={{ textAlign:'center' }}>
            ...
            <p style={{ color:C.m, fontSize:14, margin:'0 0 32px' }}>
              {t('signup.success_sub', { company: form.company })}<br/>
              {t('signup.success_sub2')}
            </p>
      ...
      {step < 3 && (
        <p style={{ marginTop:24, color:C.m, fontSize:14 }}>
          ...
          <a onClick={() => navigate('/login')} style={{ color:C.p, fontWeight:600, cursor:'pointer', textDecoration:'none' }}>
            {t('signup.login_link')}
          </a>
        </p>
      )}
