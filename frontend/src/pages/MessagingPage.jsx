...
const ROLE_BADGE = {
  ...
  partner: { label: 'Partenaire', color: '#6366f1', bg: '#eef2ff' },
};
...
export default function MessagingPage() {
  ...
  const openNewConversation = async () => {
    setShowNew(true);
    try {
      const data = await api.getMessageableUsers();
  ...
  const handleCreateConversation = async () => {
    ...
    try {
      ...
      setShowNew(false);
      setNewConvForm({ subject: '', participant_ids: [], message: '' });
  ...
  return (
    <div className="fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        ...
        <button onClick={openNewConversation} style={{
            ...
            {conversations.length === 0 ? (
              ...
              return (
                <div key={conv.id} onClick={() => openConversation(conv)} style={{
                  ...
                  onMouseLeave={e => { if (!active) e.currentTarget.style.background = unread > 0 ? '#fafbff' : '#fff'; }}
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                    ...
                    {unread > 0 && (
                      <span style={{ background: '#6366f1', color: '#fff', fontSize: 11, fontWeight: 700, borderRadius: 10, padding: '1px 8px', marginLeft: 8, flexShrink: 0 }}>
                        {unread}
                      </span>
                    )}
                  </div>
                  {conv.partner_name && (
                    <div style={{ fontSize: 11, color: '#6366f1', fontWeight: 600, marginBottom: 4 }}>{conv.partner_name}</div>
                  )}
                  {conv.last_message && (
                    <div style={{ fontSize: 12, color: '#64748b', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                      <span style={{ fontWeight: 500 }}>{conv.last_message_by}: </span>
                      {conv.last_message}
                    </div>
                  )}
            ...
            <>
                  ...
                  return (
                    <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                        ...
                        {roleBadge && !isMe && (
                          <span style={{ padding: '1px 6px', borderRadius: 4, fontSize: 10, fontWeight: 600, background: roleBadge.bg, color: roleBadge.color }}>{roleBadge.label}</span>
                        )}
              ...
              <div style={{ padding: '16px 24px', borderTop: '1px solid #e2e8f0', display: 'flex', gap: 12 }}>
                <input value={newMessage} onChange={e => setNewMessage(e.target.value)}
                  ...
                  style={{ flex: 1, padding: '10px 16px', borderRadius: 12, border: '2px solid #e2e8f0', fontSize: 14, color: '#0f172a' }} />
                <button onClick={handleSend} disabled={sending || !newMessage.trim()} style={{
                  width: 44, height: 44, borderRadius: 12, border: 'none', cursor: 'pointer',
                  background: newMessage.trim() ? 'linear-gradient(135deg,#6366f1,#8b5cf6)' : '#e2e8f0',
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div className="fade-in" style={{ position: 'relative', background: '#fff', borderRadius: 20, padding: 32, width: 520, maxWidth: '90%', maxHeight: '80vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
              ...
              <button onClick={() => setShowNew(false)} style={{ background: '#f1f5f9', border: 'none', width: 34, height: 34, borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <input value={newConvForm.subject} onChange={e => setNewConvForm(f => ({ ...f, subject: e.target.value }))}
            ...
            <div style={{ marginBottom: 20 }}>
              <label style={{ display: 'block', fontWeight: 600, color: '#334155', fontSize: 13, marginBottom: 8 }}>
                Participants * ({newConvForm.participant_ids.length} sélectionné{newConvForm.participant_ids.length > 1 ? 's' : ''})
              </label>
              <div style={{ maxHeight: 200, overflowY: 'auto', border: '2px solid #e2e8f0', borderRadius: 12, padding: 8 }}>
                {availableUsers.map(u => {
                  ...
                  return (
                    <div key={u.id} onClick={() => toggleParticipant(u.id)} style={{
                      display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px', borderRadius: 8,
                      cursor: 'pointer', background: selected ? '#eef2ff' : '#fff',
                      border: selected ? '1px solid #c7d2fe' : '1px solid transparent',
                      marginBottom: 4, transition: 'all 0.1s',
                    }}>
                      <div style={{
                        width: 18, height: 18, borderRadius: 4, border: `2px solid ${selected ? '#6366f1' : '#d1d5db'}`,
                        background: selected ? '#6366f1' : '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center',
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <textarea value={newConvForm.message} onChange={e => setNewConvForm(f => ({ ...f, message: e.target.value }))}
                placeholder="Votre message..."
                rows={4}
                style={{ width: '100%', padding: '10px 14px', borderRadius: 10, border: '2px solid #e2e8f0', fontSize: 14, resize: 'vertical', fontFamily: 'inherit', boxSizing: 'border-box' }} />
            </div>

            <button onClick={handleCreateConversation} disabled={creating || !newConvForm.subject.trim() || !newConvForm.message.trim() || newConvForm.participant_ids.length === 0}
              style={{
