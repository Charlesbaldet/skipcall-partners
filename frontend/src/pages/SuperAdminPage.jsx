...
export default function SuperAdminPage() {
  ...
  const [form, setForm] = useState({ name: '', slug: '', domain: '', primary_color: 'var(--rb-primary, #047857)', secondary_color: '#8b5cf6', accent_color: '#f59e0b', logo_url: '' });
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      setShowCreate(false); setForm({ name: '', slug: '', domain: '', primary_color: 'var(--rb-primary, #047857)', secondary_color: '#8b5cf6', accent_color: '#f59e0b', logo_url: '' }); load();
      ...
      {tab === 'tenants' && (
        <>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 16 }}>
            <button onClick={() => setShowCreate(!showCreate)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '10px 20px', borderRadius: 12, background: showCreate ? '#f1f5f9' : 'var(--rb-primary, #047857)', color: showCreate ? '#475569' : '#fff', border: 'none', fontWeight: 600, fontSize: 14, cursor: 'pointer' }}>
          ...
          {showCreate && (
            <div style={{ background: '#fff', borderRadius: 16, padding: 24, border: '1px solid #e2e8f0', marginBottom: 20 }} className="fade-in">
              ...
              <button onClick={handleCreate} disabled={saving || !form.name || !form.slug} style={{ padding: '10px 24px', borderRadius: 10, background: 'var(--rb-primary, #047857)', color: '#fff', border: 'none', fontWeight: 600, fontSize: 13, cursor: 'pointer', opacity: saving ? 0.7 : 1 }}>{saving ? 'Création...' : 'Créer le tenant'}</button>
            </div>
          )}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              ...
              <tbody>{tenants.map(t => (
                <tr key={t.id} style={{ borderBottom: '1px solid #f8fafc' }}>
                  ...
                  <td style={{ padding: '13px 16px', textAlign: 'center' }}><code style={{ fontSize: 12, color: 'var(--rb-primary, #047857)', background: '#eef2ff', padding: '2px 8px', borderRadius: 4 }}>{t.slug}</code></td>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            ...
            <tbody>{logs.map(l => (
              <tr key={l.id} style={{ borderBottom: '1px solid #f8fafc' }}>
                ...
                <td style={{ padding: '10px 14px', color: 'var(--rb-primary, #047857)', fontSize: 12 }}>{l.tenant_name || '—'}</td>
