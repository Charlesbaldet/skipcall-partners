...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    setLoading(true);
    try { await api.request('/auth/forgot-password', { method:'POST', body:JSON.stringify({ email }) }); } catch(_){}
  ...
  if (sent) return (
    <div style={{ minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:24,background:C.bg }}>
      <div style={{ maxWidth:440,width:'100%',textAlign:'center',background:'#fff',borderRadius:24,padding:48,boxShadow:'0 20px 60px rgba(0,0,0,0.08)' }}>
        ...
        <p style={{ color:C.m,marginBottom:32,lineHeight:1.6 }}>{t('forgotPwd.sent_text')}</p>
        <Link to="/login" style={{ color:C.p,fontWeight:700,textDecoration:'none',fontSize:15 }}>{t('forgotPwd.back_to_login')}</Link>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ minHeight:'100vh',display:'flex',alignItems:'center',justifyContent:'center',padding:24,background:C.bg }}>
      <div style={{ width:'100%',maxWidth:440,background:'#fff',borderRadius:24,padding:48,boxShadow:'0 20px 60px rgba(0,0,0,0.08)' }}>
        <Link to="/login" style={{ color:C.m,textDecoration:'none',fontSize:14,display:'inline-flex',alignItems:'center',gap:4,marginBottom:28 }}>{t('forgotPwd.back_to_login')}</Link>
        <h1 style={{ fontWeight:800,fontSize:28,color:C.s,margin:'0 0 8px' }}>{t('forgotPwd.title')}</h1>
        <p style={{ color:C.m,margin:'0 0 32px',lineHeight:1.6 }}>{t('forgotPwd.subtitle')}</p>
        <form onSubmit={handleSubmit}>
          <label style={{ display:'block',fontWeight:600,fontSize:13,color:C.s,marginBottom:8 }}>{t('forgotPwd.email')}</label>
          <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder={t('forgotPwd.email_ph')} required
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? t('forgotPwd.loading') : t('forgotPwd.submit')}
          </button>
        </form>
      </div>
    </div>
