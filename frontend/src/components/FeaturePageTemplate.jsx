...
const FEATURE_LINKS = [
  { href: '/fonctionnalites/pipeline', label: 'Pipeline de leads' },
...
export default function FeaturePageTemplate({
  helmet, accentColor, label, title, subtitle, mockupSvg, benefits, quote, currentHref,
}) {
    ...
    <LandingLayout>
      ...
      <section style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 100%)', padding: mobile ? '48px 20px 0' : '80px 48px 0', overflow:'hidden' }}>
        <div style={{ maxWidth:1100, margin:'0 auto', display: mobile ? 'block' : 'grid', gridTemplateColumns:'1fr 1fr', gap:64, alignItems:'flex-end' }}>
          <div style={{ paddingBottom: mobile ? 40 : 80 }}>
            ...
            <h1 style={{ margin:'0 0 16px',fontSize: mobile ? 32 : 44,fontWeight:900,color:'#fff',lineHeight:1.1,letterSpacing:-1 }}>
              {title}
            </h1>
            <p style={{ margin:'0 0 36px',fontSize: mobile ? 16 : 19,color:'#94a3b8',lineHeight:1.65 }}>
              {subtitle}
            </p>
      ...
      <section style={{ background:'#fff',padding: mobile ? '48px 20px' : '80px 48px' }}>
        <div style={{ maxWidth:1100,margin:'0 auto' }}>
          <div style={{ textAlign:'center',marginBottom: mobile ? 48 : 72 }}>
            ...
            <h2 style={{ margin:0,fontSize: mobile ? 26 : 36,fontWeight:800,color:'#0f172a',letterSpacing:-0.5 }}>Ce que vous obtenez concrètement</h2>
          </div>
          {benefits.map((b,i) => (
      ...
      <section style={{ background:'linear-gradient(135deg, #0f172a, #1e293b)',padding: mobile ? '48px 20px' : '80px 48px' }}>
        <div style={{ maxWidth:760,margin:'0 auto',textAlign:'center' }}>
          ...
          <p style={{ margin:'0 0 24px',fontSize: mobile ? 17 : 22,color:'#fff',lineHeight:1.65,fontStyle:'italic',fontWeight:300 }}>
            {quote.text}
          </p>
      ...
      <section style={{ background:'#f8fafc',padding: mobile ? '48px 20px' : '80px 48px',textAlign:'center' }}>
        <div style={{ maxWidth:560,margin:'0 auto' }}>
          ...
          <p style={{ marginTop:12,fontSize:13,color:'#94a3b8' }}>Sans engagement · 14 jours gratuits · Support inclus</p>
        </div>
