...
// Tenant isolation helper : append tenant filter to a query
function tenantFilter(req, sql, params, alias = 'c') {
  if (req.tenantId && !req.skipTenantFilter) {
    params.push(req.tenantId);
    sql += (sql.includes('WHERE') ? ' AND ' : ' WHERE ') + `${alias}.tenant_id = $${params.length}`;
  }
  return sql;
}

router.get('/config', authenticate, tenantScope, authorize('admin'), (req, res) => {
  res.json({ configured: mollie.isConfigured() });
});

router.post('/create/:commissionId', authenticate, tenantScope, authorize('admin'), async (req, res) => {
  try {
    // Tenant-scoped commission lookup
    let sql = `SELECT c.*, p.name as partner_name, r.prospect_name
               FROM commissions c
               JOIN partners p ON c.partner_id = p.id
               JOIN referrals r ON c.referral_id = r.id
               WHERE c.id = $1`;
    let params = [req.params.commissionId];
    ...
    let partnerParams = [commission.partner_id];
    if (req.tenantId && !req.skipTenantFilter) {
      partnerSql += ' AND tenant_id = $2';
      partnerParams.push(req.tenantId);
    }
    const { rows: [partner] } = await query(partnerSql, partnerParams);

    if (!partner) return res.status(404).json({ error: 'Partner introuvable' });

    const payment = await mollie.createPayout(commission, partner);
    await query(
      'UPDATE commissions SET mollie_payment_id = $2, payment_status = $3 WHERE id = $1',
...
router.get('/status/:commissionId', authenticate, tenantScope, authorize('admin'), async (req, res) => {
  try {
    let sql = 'SELECT mollie_payment_id, payment_status FROM commissions WHERE id = $1';
    let params = [req.params.commissionId];
